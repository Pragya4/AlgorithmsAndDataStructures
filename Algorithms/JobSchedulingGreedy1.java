...
class JobSchedulingGreedy1{		//first to finish strategy
...
public static void main(String args[]){
	...
	for(int i=0;i<n;i++){
		if(cur==0){
			...
			cur++;	
			count++;
			System.out.println("Job "+i+"with "+start[i]+", "+end[i]+" Scheduled in machine 0 \n");
			...
			for(int mac=0;mac<=cur;mac++){
				if(start[i]>machine[mac]){	//can allocate any of the existing machine
					machine[mac]=end[i];
					System.out.println("Job "+i+"with "+start[i]+", "+end[i]+" Scheduled in machine "+mac+"\n");
					break;			//break the for loop					
				}
				else if(mac==cur){
					if(cur<m){
						machine[cur]=end[i];	//new machine allocated
						System.out.println("Job "+i+"with "+start[i]+", "+end[i]+" Scheduled in machine "+cur+ "\n");
						cur++;
						count++;
						break;
						}
